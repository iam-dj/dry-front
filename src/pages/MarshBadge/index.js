...
export default function BoulderBadge(props) {
  ...
  useEffect(() => {
    const fetchCurrentGymStage = async () => {
      ...
      setCurrentGymStage(mainPokemon[0].gymSixStage);
    };
  ...
  const handleButtonClick = (buttonId) => {
    ...
    const generateBattle = async () => {
      ...
      try {
        ...
        let gymStage = myFilteredPokemons[0].gymSixStage;
        ...
        setTimeout(() => {
          ...
          const { result, battleLogData } = BattleSys.startBattle(
            ...
            GymLeader[0].name,
            isGymMaster,
            myFilteredPokemons[0].marshBadgeVictory
  ...
  return (
    <div style={cardStyle}>
      ...
      <div className="row">
        <div className="col">
          <img style={imgStyle} src={sabrina} alt="Brock gym leader" />
          <p
